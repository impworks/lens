...
Блоки выделяются отступами. Несколько команд в одну строку записываются через
точку с запятой (это удобно, например, в лямбдах).
...
* В языке нет возможности создавать свои классы с методами
...
* Нет pattern-matching'а
* Нет операторов возведения в степень и сокращенного присваивания
