...
ни одного параметра, по сути она все равно будет принимать тип `unit` для вызова.
...
1. Анонимная функция замыкает переменные и константы из внешней
   области видимости.
2. Анонимная функция не может быть рекурсивной, потому что не может получить
   ссылку на себя до окончания собственного объявления.
3. У анонимной функции не обязательно указывать тип, он может быть выведен.
...
Цикл `while` всегда возвращает `unit`.
...
Тип коллекции неявно выводится из первого элемента, значение которого не равно null.
Остальные значения будут неявно приведены к типу первого, если такое приведение возможно.
...
* Нет поддержки событий и обработчиков (хотя есть делегаты)
