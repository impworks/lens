...
namespace Lens.Compiler
{
    internal partial class Context
    {
        ...
        /// </summary>
        /// <returns>
        /// The instance of generated assembly's entry point class, which implements IScript or
        /// IAsyncScript depending on whether the script awaits at its top level.
        /// </returns>
        public object Compile(IEnumerable<NodeBase> nodes)
        {
            // a declared function has no body and a declared variable no value: an environment made
            // of declarations can be analysed, and cannot be run
            if (Options.DeclarationMode == DeclarationMode.Provide)
                throw new InvalidOperationException(CompilerMessages.ProvideModeCannotEmit);
        ...
        private void LoadTree(IEnumerable<NodeBase> nodes)
        {
            foreach (var currNode in nodes)
            {
                ...
                WithRecovery(() =>
                    {
                        if (node is DeclarationBlockNode)
                        {
                            DeclareEnvironment(node as DeclarationBlockNode);
                            return;
                        }

                        // a declaration block describes the environment the script runs in, so it
                        // belongs above the script rather than in the middle of it
                        if (!(node is UseNode))
                            _hasScriptContents = true;
            ...
            // everything the declarations are checked against is in place only now: a declaration
            // may name a record declared below it, and a function of the same name may be too
            ProcessDeclarations();
        }
        ...
        private void CreateInitialEntities(bool emit)
        {
            ...
            // the autogenerated members of a declaration are built out of its signature, so there is
            // nothing to build once the signature has failed to resolve
            if (Diagnostics.HasErrors)
                return;
        ...
        private void DeclareFunction(FunctionNode node)
        {
            ...
            {
                // reserved whether or not this script turns out to use it: which of the two names the
                // root method takes is not known until the whole script has been read
                if (node.Name.IsAnyOf(EntityNames.RunMethodName, EntityNames.RunAsyncMethodName, EntityNames.EntryPointMethodName))
        ...
        private void BuildStateMachines()
        {
            WithRecovery(() =>
                {
                    // a script that yields is not a sequence: unlike a task, which the script can
                    // hand out in place of its value, there is nothing for a sequence to mean here.
                    // Reported at the resume point itself - the script body is the whole file, and an
                    // error bound to it says nothing about where the offending keyword is
                    var yieldNode = Lowerer.FindFirst<YieldNode>(MainMethod.Body);
                    if (yieldNode != null)
                        Error(yieldNode, CompilerMessages.YieldNotInIterator);
        ...
        /// <summary>
        /// Turns the script body into a state machine driven by a task.
        ///
        /// The root method is not replaced but repurposed: it keeps its place as the one entry point
        /// the script type has, and changes what it is called and what it answers. Nothing has been
        /// resolved or registered under the old name yet - both happen when the entities are
        /// processed, which is after this.
        /// </summary>
        private void MakeScriptAsync()
        {
            MainMethod.Name = EntityNames.RunAsyncMethodName;
            MainMethod.ReturnType = TypeEntryCache.Of<Task<object>>();
            MainType.Interfaces = new[] {TypeEntryCache.Of<IAsyncScript>()};

            IsScriptAsync = true;

            AsyncBuilder.ForScriptBody(this, MainMethod).Build();
        }
        ...
        private void ProcessEntities(bool clearTypes, bool emit)
        {
            if (UnpreparedTypes.Count > 0)
            {
                foreach (var currType in UnpreparedTypes)
                {
                    var curr = currType;

                    // recorded rather than thrown: a bad signature on one declaration says nothing
                    // about the next one, and an editor wants every problem in the file at once
                    WithRecovery(() =>
                        {
                            curr.ResolveSelf();
            ...
            {
                foreach (var currContent in UnpreparedTypeContents)
                {
                    var curr = currContent;
        ...
        private void CreateEntryPoint()
        {
            ...
            var script = Expr.New(EntityNames.MainTypeName);

            // an executable has to be waited for: its process ends when Main returns, and a script
            // still waiting for something would be killed rather than finished. Blocking is safe
            // here in a way it is not in general - a console application has no synchronization
            // context to deadlock against
            var run = IsScriptAsync
                ? Expr.Invoke(Expr.Invoke(Expr.Invoke(script, EntityNames.RunAsyncMethodName), "GetAwaiter"), "GetResult")
                : Expr.Invoke(script, EntityNames.RunMethodName);
