...
val nuGetConfigFile = layout.projectDirectory.file("src/dotnet/nuget.config")

// MSBuild decides whether to rebuild by timestamp, so rewriting an unchanged file is not free.
// Declaring the rendered text as the input and the file as the output lets Gradle skip the task
// altogether when neither changed, which keeps both the timestamp and the rendering itself out of
// the way.
...
// is no NuGet feed for it. Behind a provider, because touching platformPath during configuration
// would force the SDK to be resolved even for tasks that do not need it.
val riderSdkPath: Provider<String> = provider {
...
val generateDotNetSdkProperties = tasks.register<GenerateFile>("generateDotNetSdkProperties") {
    ...
    target = dotNetSdkPropsFile
    content = riderSdkPath.map { sdk ->
        """
        <Project>
          <PropertyGroup>
            <DotNetSdkPath>$sdk</DotNetSdkPath>
          </PropertyGroup>
        </Project>
        """.trimIndent() + "\n"
    }
...
val generateNuGetConfig = tasks.register<GenerateFile>("generateNuGetConfig") {
    description = "Writes the NuGet configuration the backend project restores through."
    group = "build setup"

    dependsOn(Constants.Tasks.INITIALIZE_INTELLIJ_PLATFORM_PLUGIN)

    target = nuGetConfigFile
    content = riderSdkPath.map { sdk ->
        """
...
val compileDotNet = tasks.register<Exec>("compileDotNet") {
    ...
    // The generated props and NuGet config come in through their producing tasks, which makes the
    // dependency on them implicit. MSBuild imports both from fixed paths, and one of them lives in
    // the build directory, so only their content is meaningful here.
    inputs.files(generateDotNetSdkProperties, generateNuGetConfig)
